...
__all__ = ["PositionSource", "BeaconSource", "HyperliquidSource", "UniV3Source",
           "PROTOCOL_SOURCES", "CHAIN_SCOPED_SOURCES", "build_sources"]
...
CHAIN_SCOPED_SOURCES = {UniV3Source.name}
...
def build_sources(cfg) -> dict:
    ...
    wants_univ3 = any(UniV3Source.name in a.watch for a in cfg.addresses if a.enabled)
    if wants_univ3 and cfg.api_keys.alchemy:
        sources[UniV3Source.name] = UniV3Source(cfg.api_keys.alchemy, catalog.load())
    return sources
